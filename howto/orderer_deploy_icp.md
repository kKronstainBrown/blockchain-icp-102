...
You can choose to deploy the orderer on either the amd64 or s390x platforms. However, be aware that [Dynamic provisioning](https://kubernetes.io/docs/concepts/storage/dynamic-provisioning/){: external} is available for only AMD64 nodes in {{site.data.keyword.cloud_notm}} Private. If your cluster includes a mix of s390x and amd64 worker nodes, dynamic provisioning cannot be used.
...
1. Encode the configuration file in the base64 format by running the following commands from a terminal window:
  ...
  **Note:** It is important that the string generated by using these commands is formatted as a single line. It should look similar to:
  ...
  Save the resulting output for Step 4.
...
3. On the **General** tab, complete the following fields:
  - **Name:** Give your secret a unique name within your cluster. You will use this name when deploying your orderer. The name must be all lowercase.  
...
| `Orderer data persistence enabled` | Data will be available when the container restarts. If unchecked, all data will be lost when a failover or pod restart happens. | Checked | No |
...
| `Orderer selector label`| [Selector label](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/){: external} for your PVC. | None | No |
| `Orderer selector value`| [Selector value](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/){: external} for your PVC. | None | No |
| `Orderer storage access mode`| Specify the storage [access mode](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#access-modes){: external} for the PVC. | ReadWriteMany | Yes |
| `Orderer volume claim size`| Choose the size of disk to use, which must be at least 2 Gi. | 8Gi | Yes |
| `Orderer service type` | Used to specify whether [external ports should be exposed](https://kubernetes.io/docs/concepts/services-networking/service/#publishing-services-service-types){: external} on the peer. Select NodePort to expose the ports externally (recommended), and ClusterIP to not expose the ports. LoadBalancer and ExternalName are not supported in this release | NodePort | Yes |
